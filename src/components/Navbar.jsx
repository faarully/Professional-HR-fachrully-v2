...
const Navbar = ({ whatsappNumber }) => {
  ...
  // Kunci Scroll Body - Gunakan fixed position untuk cegah jumpy layout di mobile
  useEffect(() => {
    if (isOpen) {
...
const handleNavClick = (e, id) => {
  ...
  return (
    <>
      <nav className={`fixed top-0 w-full z-[100] transition-all duration-300 ease-in-out ${
        ...
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between relative z-[101]">
          ...
          <div className="flex items-center gap-4">
            ...
            >
              <span className={`block w-6 h-0.5 bg-slate-900 dark:bg-white transition-all duration-300 ${isOpen ? 'rotate-45 translate-y-1' : '-translate-y-1'}`}></span>
              ...
              <span className={`block w-6 h-0.5 bg-slate-900 dark:bg-white mt-1 transition-all duration-300 ${isOpen ? '-rotate-45 -translate-y-2' : 'translate-y-1'}`}></span>
            </button>
          </div>
        </div>

        {/* SVG Animasi Garis Hijau */}
        {!isOpen && (
      ...
      {/* MOBILE MENU OVERLAY - Dikeluarkan dari tag <nav> utama agar tidak bentrok padding */}
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed inset-0 bg-white dark:bg-slate-950 z-[98] flex flex-col items-center justify-center space-y-10 md:hidden"
          >
